...
\question
  ...
  \part[5] Express $\neg \exists!xP\; (x)$ in a similar way as (\ref{eq:uniq}). Provide an expression in formal notation as well as in English. Also, give an example of a true proposition $\neg\exists!x\; P(x)$ by slightly changing the one you gave in part (a).
    \begin{solution}
      ...
      \newline there is a unique $x$ such that $P(x)$” means two things; uniqueness and existence!
      \newline The negation violates both statements, which means either none exists at all, or more than one exists.
      \newline Example can be written as $"x-5\neq7"$ having a domain such that $x \in     \mathbb{R}              $ 
      \newline There can be many posssible values of x that exist, except the unique value satisfying the previous given example $"x-5=7"$ i.e. '12'!
    \end{solution}
  \end{parts}
