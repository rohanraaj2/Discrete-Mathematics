...
\question[5] A small company makes widgets in a variety of constituent materials (aluminum, copper, iron), colors (red, green, blue, grey), and finishes (matte, textured, coated). Although there are many combinations of widget features, the company markets only a subset of the possible combinations. The following sentences are constraints that characterize the possibilities. 
  ...
  \begin{solution}
    ...
    \begin{tabularx}{\textwidth}{l|l|X}
      ...
      aluminum $\lor$ copper $\lor$ iron & Satisfied & Since the metal is copper, it satisfies the truth value\\
      aluminum $\implies$ grey & Satisfied & Since the metal is copper, the order does not contradict with it. \\\hline
      copper $\land$ $\neg$ coated $\implies$ red & Satisfied & Since the metal is copper, and the finish is not coated (is matte) so red color does not contradict \\\hline
      coated $\land$ $\neg$ copper $\implies$ green & Not satisfied & Since the metal is copper, and not coated, the order contradicts as color is green \\\hline
      green $\lor$ blue $\iff \neg$ textured $\land$ $\neg$ iron & Satisfied & Since the order's colors are both green and blue, they are not textured and has no iron as well, so order doesn't contradict with constraints. 
    \end{tabularx}
  \end{solution}
...
\question An argument is said to be \textit{valid} if its \textit{conclusion} can be inferred from its \textit{premises}. An argument that is not valid is called an \textit{invalid} argument, or a \textit{fallacy}. For each of the arguments below, identify the simple propositions involved, write the premises and conclusion(s) in logical notation using the identified simple propositions, and decide whether it is valid. Justify your decision.
  ...
  \part[5] If I am wealthy, then I am happy. I am happy, therefore, I am wealthy.
    \begin{solution}
      ...
      The conlusion p cannot be inferred from its premise. Therefore, the argument is invalid since there is no evidence in the table of rules of inference.
    \end{solution}
  \part[5]
    ...
    \begin{solution}
      ...
      \[
        ...
        \end{array}
      \]
      The conclusion $\neg q$ can be inferred from its premise. The statement is logically correct as \textit{Modus Tollens}
  ...
  \part[5] If I study, then I will not fail CS 113. If I do not play cards too often, then I will study. I failed CS 113. Therefore, I played cards too often.
    \begin{solution}
      ...
      \[
        ...
        \end{array}
      \]
      The statement is logically incorrect as the conclusion $r$ cannot be inferred from its premise.
    \end{solution}
  \end{parts}
