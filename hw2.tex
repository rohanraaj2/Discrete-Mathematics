...
\question
  ...
  \part[5] $\forall x \forall y\; (x^2= y^2 \rightarrow x=y)$
    \begin{solution}
      % Enter your solution here.
      1) 1. If the squares of two integers are equal, then the integers must have already been
      equal
      2) 2. This proposition is False. A counterexample is (x, y) : (1, −1). (One counterexample is sufficient to disprove universality.)
  ...
  \part[5] $\forall x \exists y\; (y^2=x)$
    \begin{solution}
      ...
      2. This proposition is False. A counterexample is x = 2 for which no y ∈ Z can
  ...
  \part[5] $\exists x \forall y\; (x \leq y^2)$
    \begin{solution}
      ...
      the proposition True, as 0 ≤ y
      2
      for all y ∈ Z. (One example is sufficient to prove
      existence.)
      3. One other domain for which the proposition is true is R. But if we take R  
      + ,
